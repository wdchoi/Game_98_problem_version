...
public:
    ...
    QState(const VectorND<D>& _in, const int& _choide, const D& _re)
        : input_(_in), choice_(_choide), reward_(_re)
    {}

    void operator = (const QState& qs)
    {
        input_ = qs.input_;
        choice_ = qs.choice_;
        reward_ = qs.reward_;
...
public:
    ...
    CircularQueue<QState> history_;
    
    NeuralNetwork()
        //: rand(1)
    {}

    NeuralNetwork(const int& _num_input, const int& _num_output, const int& _num_hidden_layers, const int& num_records)
        //: rand(1)
    {
        initialize(_num_input, _num_output, _num_hidden_layers, num_records);
    }

    void initialize(const int& _num_input, const int& _num_output, const int& _num_hidden_layers, const int& num_records)
    {
        ...
        initialize(num_layer_acts_, _num_hidden_layers, num_records);
    }

    void initialize(const VectorND<unsigned>& num_layer_acts_, const int& _num_hidden_layers, const int& num_records)
    {
        ...
        history_.initialize(num_records);
    }
